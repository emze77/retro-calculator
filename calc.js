...
function appendNumber(number) {
  ...
  if (STATS.inputFirstOperator && STATS.firstOperator.length < 10) {
    if (
      STATS.firstOperator.length === 1 &&
      STATS.firstOperator.charAt(0) === "0"
    )
      deleteNumber();
  ...
  if (!STATS.inputFirstOperator && STATS.secondOperator.length < 10) {
    if (
      STATS.secondOperator.length === 1 &&
      STATS.secondOperator.charAt(0) === "0"
    )
      deleteNumber();
...
function toggleLed(number, color) {
  ...
  LED.list.forEach((item, index) => {
    if (index === number) {
      ...
      item.forEach((e) =>
        e.classList.remove(LED.colors[0], LED.colors[1], LED.colors[2])
      );
    }
  });
}
...
function resetWaitingTimer() {
  if (STATS.blinkingPhase) {
    if (typeof TIMEOUTS.impuls === "number") clearTimeout(TIMEOUTS.impuls);
    clearWaiting();
    resetWaitingTimer();
  }
  if (typeof TIMEOUTS.waiting === "number") clearTimeout(TIMEOUTS.waiting);
